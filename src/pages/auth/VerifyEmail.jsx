...
export default function VerifyEmail() {
  const { user, sendEmailOtp, verifyEmail } = useAuth();
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
      <Card padding="40px" hover={false} style={{ width: '100%', maxWidth: 440 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 6 }}>Verify Your Email</h1>
          <p style={{ fontSize: 14, color: 'var(--text-secondary)' }}>
            We sent an OTP to <strong>{user?.email}</strong>
          </p>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 24 }}>
          {[1, 2, 3, 4, 5].map((s) => (
            <div key={s} style={{
              ...
              backgroundColor: s <= 2 ? 'var(--primary)' : 'var(--border)',
            }} />
          ))}
          <span style={{ fontSize: 12, color: 'var(--text-muted)', marginLeft: 8 }}>Step 2 of 5</span>
        </div>
