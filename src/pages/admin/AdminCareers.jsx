...
export default function AdminCareers() {
  ...
  const [form, setForm] = useState({
    jobTitle: "", department: "", location: "", employmentType: "", experience: "",
    salary: "", description: "", responsibilities: "", requirements: "", benefits: "",
    applyLink: "", status: "draft",
  });
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/careers?${params}`, { credentials: "include" });
      if (res.ok) { const d = await res.json(); setItems(d.items || []); setTotal(d.total || 0); }
    } catch (e) { console.error(e); }
    finally { setLoading(false); }
  };

  useEffect(() => { fetchItems(); }, [page, deptFilter]);
  useEffect(() => {
    const timer = setTimeout(() => { setPage(1); fetchItems(); }, 300);
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ jobTitle: "", department: "", location: "", employmentType: "", experience: "", salary: "", description: "", responsibilities: "", requirements: "", benefits: "", applyLink: "", status: "draft" });
    setShowModal(true);
  };
  ...
  const handleDelete = async () => {
    try {
      await fetch(`${API}/careers/${deleteId}`, { method: "DELETE", credentials: "include" });
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        ...
        <button style={s.addBtn} onClick={openCreate}><Plus size={18} /> New Job</button>
      </div>

      <div style={s.searchBar}>
        <div style={{ position: "relative", flex: 1 }}>
          <Search size={16} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "var(--text-muted)" }} />
          <input style={{ ...s.searchInput, paddingLeft: 36 }} placeholder="Search by job title..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        {departments.length > 0 && (
          <select style={s.filterSelect} value={deptFilter} onChange={(e) => { setDeptFilter(e.target.value); setPage(1); }}>
      ...
      <AnimatePresence mode="popLayout">
        {items.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={s.emptyState}>
            <p style={{ fontSize: 16 }}>No job postings found</p>
            <p style={{ fontSize: 14, marginTop: 8 }}>Create your first job posting to get started.</p>
          </motion.div>
        ) : (
          items.map((item) => (
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 600, margin: 0 }}>{item.jobTitle}</h3>
                  ...
                  <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0 }}>
                    {[item.department, item.location, item.employmentType, item.experience].filter(Boolean).join(" · ")}
                  </p>
                  {item.salary && <p style={{ fontSize: 13, color: "var(--primary)", fontWeight: 600, marginTop: 4, marginBottom: 0 }}>{item.salary}</p>}
                </div>
                <div style={s.actions}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} style={s.modalContent} onClick={(e) => e.stopPropagation()}>
              <div style={s.modalHeader}>
                <h2 style={s.modalTitle}>{editing ? "Edit Job" : "New Job"}</h2>
              ...
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input style={s.input} value={form.jobTitle} onChange={(e) => setForm((prev) => ({ ...prev, jobTitle: e.target.value }))} />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Department</label>
                    <input style={s.input} value={form.department} onChange={(e) => setForm((prev) => ({ ...prev, department: e.target.value }))} />
                  </div>
                </div>
              </div>
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input style={s.input} value={form.location} onChange={(e) => setForm((prev) => ({ ...prev, location: e.target.value }))} />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Employment Type</label>
                    <select style={s.select} value={form.employmentType} onChange={(e) => setForm((prev) => ({ ...prev, employmentType: e.target.value }))}>
              ...
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input style={s.input} value={form.experience} onChange={(e) => setForm((prev) => ({ ...prev, experience: e.target.value }))} placeholder="e.g. 2-3 years" />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Salary</label>
                    <input style={s.input} value={form.salary} onChange={(e) => setForm((prev) => ({ ...prev, salary: e.target.value }))} placeholder="e.g. $50k-$70k" />
                  </div>
                </div>
              </div>
              <div style={s.fieldGroup}>
              ...
              <div style={s.fieldGroup}>
                <label style={s.label}>Responsibilities</label>
                <textarea style={s.textarea} value={form.responsibilities} onChange={(e) => setForm((prev) => ({ ...prev, responsibilities: e.target.value }))} />
              </div>
              <div style={s.fieldGroup}>
                <label style={s.label}>Requirements</label>
                <textarea style={s.textarea} value={form.requirements} onChange={(e) => setForm((prev) => ({ ...prev, requirements: e.target.value }))} />
              </div>
              <div style={s.fieldGroup}>
                <label style={s.label}>Benefits</label>
                <textarea style={s.textarea} value={form.benefits} onChange={(e) => setForm((prev) => ({ ...prev, benefits: e.target.value }))} />
              </div>
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Apply Link</label>
                    <input style={s.input} value={form.applyLink} onChange={(e) => setForm((prev) => ({ ...prev, applyLink: e.target.value }))} />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
        ...
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setDeleteId(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} style={{ ...s.modalContent, maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
              ...
              <p style={{ fontSize: 14, color: "var(--text-secondary)", marginBottom: 24 }}>Are you sure you want to delete this job posting?</p>
              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
