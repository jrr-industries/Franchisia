...
export default function AdminPartnersEditor() {
  ...
  const [form, setForm] = useState({
    logo: "", name: "", partnerType: "", description: "", website: "", isFeatured: false, displayOrder: 0, status: "draft",
  });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ logo: "", name: "", partnerType: "", description: "", website: "", isFeatured: false, displayOrder: 0, status: "draft" });
    setShowModal(true);
  };
  ...
  const openEdit = (item) => {
    setEditing(item);
    setForm({
      logo: item.logo || "", name: item.name || "", partnerType: item.partnerType || "", description: item.description || "",
      website: item.website || "", isFeatured: item.isFeatured || false, displayOrder: item.displayOrder ?? 0, status: item.status || "draft",
    });
    setShowModal(true);
  };
            ...
            <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} style={s.card}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 16, flex: 1 }}>
                  {item.logo ? (
                    <img src={item.logo} alt={item.name} style={{ width: 48, height: 48, borderRadius: 8, objectFit: "cover" }} />
                  ) : (
                    <div style={{ width: 48, height: 48, borderRadius: 8, backgroundColor: "var(--border)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-muted)", fontSize: 18, fontWeight: 600 }}>{item.name?.charAt(0) || "?"}</div>
                  )}
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      <h3 style={{ fontSize: 16, fontWeight: 600, margin: 0 }}>{item.name}</h3>
                      <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, backgroundColor: item.isFeatured ? "#DBEAFE" : "#F3F4F6", color: item.isFeatured ? "#1E40AF" : "#374151" }}>{item.isFeatured ? "Featured" : "Standard"}</span>
                    ...
                    <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0 }}>{item.partnerType}{item.website ? ` | ${item.website}` : ""} | Order: {item.displayOrder}</p>
                  </div>
                </div>
                <div style={s.actions}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} style={s.modalContent} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input style={s.input} value={form.partnerType} onChange={(e) => setForm((prev) => ({ ...prev, partnerType: e.target.value }))} />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
              ...
              <div style={s.checkboxRow}>
                ...
                <label htmlFor="isFeatured" style={s.label}>Featured Partner</label>
              </div>
