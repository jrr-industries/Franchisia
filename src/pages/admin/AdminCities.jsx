...
const s = {
  ...
  imagePreview: { width: 48, height: 48, borderRadius: 8, objectFit: "cover", border: "1px solid var(--border)" },
...
export default function AdminCities() {
  ...
  const [form, setForm] = useState({
    name: "", state: "", image: "", isFeatured: false, displayOrder: 0, listingCount: 0,
  });
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/featured-cities?${params}`, { credentials: "include" });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ name: "", state: "", image: "", isFeatured: false, displayOrder: 0, listingCount: 0 });
    setShowModal(true);
  };
  ...
  const openEdit = (item) => {
    ...
    setForm({
      ...
      isFeatured: item.isFeatured ?? false, displayOrder: item.displayOrder ?? 0, listingCount: item.listingCount ?? 0,
    });
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const body = { ...form, displayOrder: parseInt(form.displayOrder) || 0, listingCount: parseInt(form.listingCount) || 0 };
      const url = editing ? `${API}/featured-cities/${editing.id}` : `${API}/featured-cities`;
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        <h1 style={s.title}>Featured Cities</h1>
        <button style={s.addBtn} onClick={openCreate}><Plus size={18} /> New City</button>
      </div>

      <AnimatePresence mode="popLayout">
        {items.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={s.emptyState}>
            <p style={{ fontSize: 16 }}>No featured cities found</p>
            <p style={{ fontSize: 14, marginTop: 8 }}>Add your first featured city to get started.</p>
          </motion.div>
        ) : (
          items.map((item) => (
            <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} style={s.card}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12, flex: 1 }}>
                  {item.image ? (
                    <img src={item.image} alt={item.name} style={s.imagePreview} onError={(e) => { e.target.style.display = "none"; }} />
                  ) : (
                    <div style={{ ...s.imagePreview, backgroundColor: "var(--border)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-muted)", fontSize: 18, fontWeight: 600 }}>
                      {item.name?.charAt(0) || "?"}
                    </div>
                  )}
                  <div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                      ...
                      {item.state && <span style={{ fontSize: 13, color: "var(--text-muted)" }}>{item.state}</span>}
                      {item.isFeatured && <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, backgroundColor: "#DBEAFE", color: "#1E40AF" }}>Featured</span>}
                    </div>
                    <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0 }}>Order: {item.displayOrder} | Listings: {item.listingCount}</p>
                  </div>
                </div>
                <div style={s.actions}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} style={s.modalContent} onClick={(e) => e.stopPropagation()}>
              <div style={s.modalHeader}>
                <h2 style={s.modalTitle}>{editing ? "Edit City" : "New City"}</h2>
              ...
              <div style={s.formRow}>
                ...
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input style={s.input} value={form.state} onChange={(e) => setForm((prev) => ({ ...prev, state: e.target.value }))} placeholder="CA" />
                  </div>
                </div>
              </div>
              <div style={s.fieldGroup}>
              ...
              <div style={s.formRow}>
                ...
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Listing Count</label>
                    <input style={s.input} type="number" value={form.listingCount} onChange={(e) => setForm((prev) => ({ ...prev, listingCount: e.target.value }))} />
                  </div>
                </div>
              </div>
        ...
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setDeleteId(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} style={{ ...s.modalContent, maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
              ...
              <p style={{ fontSize: 14, color: "var(--text-secondary)", marginBottom: 24 }}>Are you sure you want to delete this featured city?</p>
              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
