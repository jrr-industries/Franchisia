...
export default function AdminHeroSlides() {
  ...
  const [form, setForm] = useState({
    title: "", subtitle: "", quote: "", backgroundType: "color", backgroundColor: "",
    backgroundImage: "", ctaText: "", ctaUrl: "", secondaryCtaText: "", secondaryCtaUrl: "",
    heroImage: "", isActive: true, sortOrder: 0,
  });
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/hero-slides?${params}`, { credentials: "include" });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ title: "", subtitle: "", quote: "", backgroundType: "color", backgroundColor: "", backgroundImage: "", ctaText: "", ctaUrl: "", secondaryCtaText: "", secondaryCtaUrl: "", heroImage: "", isActive: true, sortOrder: 0 });
    setShowModal(true);
  };
  ...
  const openEdit = (item) => {
    setEditing(item);
    setForm({
      title: item.title || "", subtitle: item.subtitle || "", quote: item.quote || "",
      backgroundType: item.backgroundType || "color", backgroundColor: item.backgroundColor || "",
      backgroundImage: item.backgroundImage || "", ctaText: item.ctaText || "", ctaUrl: item.ctaUrl || "",
      secondaryCtaText: item.secondaryCtaText || "", secondaryCtaUrl: item.secondaryCtaUrl || "",
      heroImage: item.heroImage || "", isActive: item.isActive ?? true, sortOrder: item.sortOrder ?? 0,
    });
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = editing ? `${API}/hero-slides/${editing.id}` : `${API}/hero-slides`;
  ...
  const handleDelete = async () => {
    try {
      await fetch(`${API}/hero-slides/${deleteId}`, { method: "DELETE", credentials: "include" });
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        ...
        <button style={s.addBtn} onClick={openCreate}><Plus size={18} /> New Slide</button>
      </div>

      <AnimatePresence mode="popLayout">
        {items.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={s.emptyState}>
            <p style={{ fontSize: 16 }}>No hero slides found</p>
            <p style={{ fontSize: 14, marginTop: 8 }}>Create your first hero slide to get started.</p>
          </motion.div>
        ) : (
          items.map((item) => (
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 600, margin: 0 }}>{item.title || "Untitled"}</h3>
                    <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, backgroundColor: item.isActive ? "#D1FAE5" : "#F3F4F6", color: item.isActive ? "#065F46" : "#374151" }}>{item.isActive ? "Active" : "Inactive"}</span>
                  </div>
                  <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0 }}>Type: {item.backgroundType} | Sort: {item.sortOrder}</p>
                  {item.subtitle && <p style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 4, marginBottom: 0 }}>{item.subtitle}</p>}
                </div>
                <div style={s.actions}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} style={s.modalContent} onClick={(e) => e.stopPropagation()}>
              <div style={s.modalHeader}>
                <h2 style={s.modalTitle}>{editing ? "Edit Hero Slide" : "New Hero Slide"}</h2>
              ...
              <div style={s.fieldGroup}>
                ...
                <textarea style={s.textarea} value={form.quote} onChange={(e) => setForm((prev) => ({ ...prev, quote: e.target.value }))} />
              </div>
              <div style={s.fieldGroup}>
                <label style={s.label}>Background Type</label>
                <select style={s.select} value={form.backgroundType} onChange={(e) => setForm((prev) => ({ ...prev, backgroundType: e.target.value }))}>
              ...
              {form.backgroundType === "color" && (
                <div style={s.fieldGroup}>
                  ...
                  <input style={s.input} value={form.backgroundColor} onChange={(e) => setForm((prev) => ({ ...prev, backgroundColor: e.target.value }))} placeholder="#000000" />
                </div>
              )}
              ...
              <div style={s.fieldGroup}>
                ...
                <input style={s.input} value={form.heroImage} onChange={(e) => setForm((prev) => ({ ...prev, heroImage: e.target.value }))} />
              </div>
              <div style={s.formRow}>
        ...
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setDeleteId(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} style={{ ...s.modalContent, maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
              ...
              <p style={{ fontSize: 14, color: "var(--text-secondary)", marginBottom: 24 }}>Are you sure you want to delete this hero slide?</p>
              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
