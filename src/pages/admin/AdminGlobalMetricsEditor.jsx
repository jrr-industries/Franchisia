...
export default function AdminGlobalMetricsEditor() {
  ...
  const [form, setForm] = useState({
    label: "", value: "", icon: "", prefix: "", suffix: "",
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/global-metrics`, { credentials: "include" });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ label: "", value: "", icon: "", prefix: "", suffix: "", displayOrder: items.length + 1, isActive: true });
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = editing ? `${API}/global-metrics/${editing.id}` : `${API}/global-metrics`;
  ...
  const handleDelete = async () => {
    try {
      await fetch(`${API}/global-metrics/${deleteId}`, { method: "DELETE", credentials: "include" });
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        <h1 style={s.title}>Global Metrics</h1>
        <button style={s.addBtn} onClick={openCreate}><Plus size={18} /> Add Metric</button>
      </div>

      <AnimatePresence mode="popLayout">
        {items.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={s.emptyState}>
            <p style={{ fontSize: 16 }}>No metrics found</p>
            <p style={{ fontSize: 14, marginTop: 8 }}>Add your first global metric to get started.</p>
          </motion.div>
        ) : (
          items.map((item) => (
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <h3 style={{ fontSize: 18, fontWeight: 700, margin: 0, color: "var(--primary)" }}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} style={s.modalContent} onClick={(e) => e.stopPropagation()}>
              <div style={s.modalHeader}>
                <h2 style={s.modalTitle}>{editing ? "Edit Metric" : "New Metric"}</h2>
              ...
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input style={s.input} value={form.value} onChange={(e) => setForm((p) => ({ ...p, value: e.target.value }))} placeholder="e.g. 10,000+" />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
              ...
              <div style={s.fieldGroup}>
                ...
                <input style={s.input} value={form.label} onChange={(e) => setForm((p) => ({ ...p, label: e.target.value }))} placeholder="e.g. Active Franchise Listings" />
              </div>
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Prefix</label>
                    <input style={s.input} value={form.prefix} onChange={(e) => setForm((p) => ({ ...p, prefix: e.target.value }))} placeholder="e.g. $" />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Suffix</label>
                    <input style={s.input} value={form.suffix} onChange={(e) => setForm((p) => ({ ...p, suffix: e.target.value }))} placeholder="e.g. +" />
                  </div>
                </div>
              </div>
              <div style={s.fieldGroup}>
                <label style={s.label}>Icon (emoji or URL)</label>
                <input style={s.input} value={form.icon} onChange={(e) => setForm((p) => ({ ...p, icon: e.target.value }))} />
              </div>
              <div style={s.checkboxRow}>
                <input type="checkbox" id="gmIsActive" checked={form.isActive} onChange={(e) => setForm((p) => ({ ...p, isActive: e.target.checked }))} />
                <label htmlFor="gmIsActive" style={s.label}>Active</label>
              </div>
        ...
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setDeleteId(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} style={{ ...s.modalContent, maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
              ...
              <p style={{ fontSize: 14, color: "var(--text-secondary)", marginBottom: 24 }}>Are you sure you want to delete this metric?</p>
              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
