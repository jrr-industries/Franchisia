...
export default function AdminEventsEditor() {
  ...
  const [form, setForm] = useState({
    title: "", banner: "", description: "", venue: "", address: "", googleMapLink: "",
    date: "", time: "", organizer: "", registrationLink: "", maxParticipants: "", status: "upcoming",
  });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ title: "", banner: "", description: "", venue: "", address: "", googleMapLink: "", date: "", time: "", organizer: "", registrationLink: "", maxParticipants: "", status: "upcoming" });
    setShowModal(true);
  };
  ...
  const handleDelete = async () => {
    try {
      await fetch(`${API}/events/${deleteId}`, { method: "DELETE", credentials: "include" });
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        ...
        <button style={s.addBtn} onClick={openCreate}><Plus size={18} /> New Event</button>
      </div>
      ...
      <AnimatePresence mode="popLayout">
        {items.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={s.emptyState}>
            ...
            <p style={{ fontSize: 14, marginTop: 8 }}>Create your first event to get started.</p>
          </motion.div>
        ) : (
          items.map((item, index) => (
            <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} style={s.card}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                ...
                {item.banner && <img src={item.banner} alt="" style={{ width: 80, height: 60, borderRadius: 8, objectFit: "cover" }} />}
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0 }}>
                    {[item.date ? new Date(item.date).toLocaleDateString() : "", item.time, item.venue, item.organizer].filter(Boolean).join(" · ")}
                  </p>
                </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} style={s.modalContent} onClick={(e) => e.stopPropagation()}>
              <div style={s.modalHeader}>
                <h2 style={s.modalTitle}>{editing ? "Edit Event" : "New Event"}</h2>
              ...
              <div style={s.fieldGroup}>
                <label style={s.label}>Banner URL</label>
                <input style={s.input} value={form.banner} onChange={(e) => setForm((prev) => ({ ...prev, banner: e.target.value }))} />
              </div>
              <div style={s.fieldGroup}>
              ...
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input style={s.input} value={form.venue} onChange={(e) => setForm((prev) => ({ ...prev, venue: e.target.value }))} />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
              ...
              <div style={s.fieldGroup}>
                <label style={s.label}>Google Map Link</label>
                <input style={s.input} value={form.googleMapLink} onChange={(e) => setForm((prev) => ({ ...prev, googleMapLink: e.target.value }))} />
              </div>
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Date</label>
                    <input type="date" style={s.input} value={form.date} onChange={(e) => setForm((prev) => ({ ...prev, date: e.target.value }))} />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Time</label>
                    <input style={s.input} value={form.time} onChange={(e) => setForm((prev) => ({ ...prev, time: e.target.value }))} />
                  </div>
                </div>
              </div>
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input style={s.input} value={form.organizer} onChange={(e) => setForm((prev) => ({ ...prev, organizer: e.target.value }))} />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Registration Link</label>
                    <input style={s.input} value={form.registrationLink} onChange={(e) => setForm((prev) => ({ ...prev, registrationLink: e.target.value }))} />
                  </div>
                </div>
              </div>
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input type="number" style={s.input} value={form.maxParticipants} onChange={(e) => setForm((prev) => ({ ...prev, maxParticipants: e.target.value }))} />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
        ...
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setDeleteId(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} style={{ ...s.modalContent, maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
              ...
              <p style={{ fontSize: 14, color: "var(--text-secondary)", marginBottom: 24 }}>Are you sure you want to delete this event?</p>
              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
