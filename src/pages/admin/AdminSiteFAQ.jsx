...
export default function AdminSiteFAQ() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/faq?${params}`, { credentials: "include" });
      if (res.ok) { const d = await res.json(); setItems(d.items || []); setTotal(d.total || 0); }
    } catch (e) { console.error(e); }
    finally { setLoading(false); }
  };

  useEffect(() => { fetchItems(); }, [page, categoryFilter]);

  const categories = [...new Set(items.map((i) => i.category).filter(Boolean))];

  const openCreate = () => {
    setEditing(null);
    setForm({ question: "", answer: "", category: "", displayOrder: items.length + 1 });
    setShowModal(true);
  };
  ...
  const handleDelete = async () => {
    try {
      await fetch(`${API}/faq/${deleteId}`, { method: "DELETE", credentials: "include" });
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        <h1 style={s.title}>Site FAQs</h1>
        <button style={s.addBtn} onClick={openCreate}><Plus size={18} /> New FAQ</button>
      </div>

      <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
        <select style={s.filterSelect} value={categoryFilter} onChange={(e) => { setCategoryFilter(e.target.value); setPage(1); }}>
          <option value="">All Categories</option>
          {categories.length > 0 ? categories.map((c) => <option key={c} value={c}>{c}</option>) : null}
      ...
      <AnimatePresence mode="popLayout">
        {items.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={s.emptyState}>
            <p style={{ fontSize: 16 }}>No FAQs found</p>
            <p style={{ fontSize: 14, marginTop: 8 }}>Create your first FAQ to get started.</p>
          </motion.div>
        ) : (
          items.map((item) => (
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <h3 style={{ fontSize: 15, fontWeight: 600, margin: 0 }}>{item.question}</h3>
                    {item.category && <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, backgroundColor: "#F3F4F6", color: "#374151" }}>{item.category}</span>}
                  </div>
                  <p style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 6, marginBottom: 0 }}>{item.answer}</p>
                  <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 6, marginBottom: 0 }}>Order: {item.displayOrder ?? 0}</p>
                </div>
                <div style={s.actions}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} style={s.modalContent} onClick={(e) => e.stopPropagation()}>
              <div style={s.modalHeader}>
                <h2 style={s.modalTitle}>{editing ? "Edit FAQ" : "New FAQ"}</h2>
                <button style={s.closeBtn} onClick={() => setShowModal(false)}><X size={20} /></button>
              </div>
              <div style={s.fieldGroup}>
                <label style={s.label}>Question</label>
                <input style={s.input} value={form.question} onChange={(e) => setForm((prev) => ({ ...prev, question: e.target.value }))} />
              </div>
              <div style={s.fieldGroup}>
                <label style={s.label}>Answer</label>
                <textarea style={s.textarea} value={form.answer} onChange={(e) => setForm((prev) => ({ ...prev, answer: e.target.value }))} />
              </div>
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Category</label>
                    <input style={s.input} value={form.category} onChange={(e) => setForm((prev) => ({ ...prev, category: e.target.value }))} placeholder="e.g. General, Account" />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
        ...
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setDeleteId(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} style={{ ...s.modalContent, maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
              ...
              <p style={{ fontSize: 14, color: "var(--text-secondary)", marginBottom: 24 }}>Are you sure you want to delete this FAQ?</p>
              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
