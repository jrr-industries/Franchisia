...
export default function AdminHowItWorksEditor() {
  ...
  const [form, setForm] = useState({
    stepNumber: 1, title: "", description: "", icon: "", illustration: "", animation: "", isPublished: true,
  });
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/how-it-works?${params}`, { credentials: "include" });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ stepNumber: 1, title: "", description: "", icon: "", illustration: "", animation: "", isPublished: true });
    setShowModal(true);
  };
  ...
  const openEdit = (item) => {
    setEditing(item);
    setForm({
      stepNumber: item.stepNumber || 1, title: item.title || "", description: item.description || "",
      icon: item.icon || "", illustration: item.illustration || "", animation: item.animation || "",
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const body = { ...form, stepNumber: parseInt(form.stepNumber) || 1 };
      const url = editing ? `${API}/how-it-works/${editing.id}` : `${API}/how-it-works`;
  ...
  const handleDelete = async () => {
    try {
      await fetch(`${API}/how-it-works/${deleteId}`, { method: "DELETE", credentials: "include" });
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        <h1 style={s.title}>How It Works Editor</h1>
        <button style={s.addBtn} onClick={openCreate}><Plus size={18} /> New Step</button>
      </div>

      <AnimatePresence mode="popLayout">
        {items.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={s.emptyState}>
            <p style={{ fontSize: 16 }}>No steps found</p>
            <p style={{ fontSize: 14, marginTop: 8 }}>Create your first step to get started.</p>
          </motion.div>
        ) : (
          items.map((item) => (
            <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} style={s.card}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 16, flex: 1 }}>
                  <div style={{ width: 40, height: 40, borderRadius: "50%", backgroundColor: "var(--primary)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 700, flexShrink: 0 }}>
                    {item.stepNumber}
                  </div>
                  <div>
                    ...
                    {item.description && <p style={{ fontSize: 13, color: "var(--text-secondary)", margin: 0 }}>{item.description}</p>}
                    <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4, marginBottom: 0 }}>Icon: {item.icon || "none"}</p>
                  </div>
                </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} style={s.modalContent} onClick={(e) => e.stopPropagation()}>
              <div style={s.modalHeader}>
                <h2 style={s.modalTitle}>{editing ? "Edit Step" : "New Step"}</h2>
                <button style={s.closeBtn} onClick={() => setShowModal(false)}><X size={20} /></button>
              </div>
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Step Number</label>
                    <input style={s.input} type="number" value={form.stepNumber} onChange={(e) => setForm((prev) => ({ ...prev, stepNumber: e.target.value }))} />
                  </div>
                </div>
              ...
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input style={s.input} value={form.icon} onChange={(e) => setForm((prev) => ({ ...prev, icon: e.target.value }))} placeholder="FaSearch" />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Animation</label>
                    <input style={s.input} value={form.animation} onChange={(e) => setForm((prev) => ({ ...prev, animation: e.target.value }))} placeholder="fadeInUp" />
                  </div>
                </div>
              </div>
              <div style={s.fieldGroup}>
                <label style={s.label}>Illustration URL</label>
                <input style={s.input} value={form.illustration} onChange={(e) => setForm((prev) => ({ ...prev, illustration: e.target.value }))} />
              </div>
        ...
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setDeleteId(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} style={{ ...s.modalContent, maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
              ...
              <p style={{ fontSize: 14, color: "var(--text-secondary)", marginBottom: 24 }}>Are you sure you want to delete this step?</p>
              <div style={s.saveBtnRow}>
