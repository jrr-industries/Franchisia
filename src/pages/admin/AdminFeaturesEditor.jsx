...
export default function AdminFeaturesEditor() {
  ...
  const [form, setForm] = useState({
    icon: "", title: "", description: "", category: "", buttonText: "", buttonUrl: "", image: "", isPublished: true, sortOrder: 0,
  });
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/features?${params}`, { credentials: "include" });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ icon: "", title: "", description: "", category: "", buttonText: "", buttonUrl: "", image: "", isPublished: true, sortOrder: 0 });
    setShowModal(true);
  };
  ...
  const openEdit = (item) => {
    ...
    setForm({
      ...
      category: item.category || "", buttonText: item.buttonText || "", buttonUrl: item.buttonUrl || "",
      image: item.image || "", isPublished: item.isPublished ?? true, sortOrder: item.sortOrder ?? 0,
    });
    setShowModal(true);
  };
  ...
  const handleDelete = async () => {
    try {
      await fetch(`${API}/features/${deleteId}`, { method: "DELETE", credentials: "include" });
  ...
  return (
    <div style={s.page}>
      <div style={s.header}>
        <h1 style={s.title}>Features Editor</h1>
        <button style={s.addBtn} onClick={openCreate}><Plus size={18} /> New Feature</button>
      </div>

      <AnimatePresence mode="popLayout">
        {items.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={s.emptyState}>
            <p style={{ fontSize: 16 }}>No features found</p>
            <p style={{ fontSize: 14, marginTop: 8 }}>Create your first feature to get started.</p>
          </motion.div>
        ) : (
          items.map((item) => (
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    ...
                    {item.category && <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, backgroundColor: "#DBEAFE", color: "#1E40AF" }}>{item.category}</span>}
                  </div>
                  <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0 }}>Sort: {item.sortOrder}</p>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} style={s.modalContent} onClick={(e) => e.stopPropagation()}>
              <div style={s.modalHeader}>
                <h2 style={s.modalTitle}>{editing ? "Edit Feature" : "New Feature"}</h2>
              ...
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input style={s.input} value={form.icon} onChange={(e) => setForm((prev) => ({ ...prev, icon: e.target.value }))} placeholder="FaRocket" />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    <label style={s.label}>Category</label>
                    <input style={s.input} value={form.category} onChange={(e) => setForm((prev) => ({ ...prev, category: e.target.value }))} placeholder="Core" />
                  </div>
                </div>
              </div>
              <div style={s.fieldGroup}>
        ...
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setDeleteId(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} style={{ ...s.modalContent, maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
              ...
              <p style={{ fontSize: 14, color: "var(--text-secondary)", marginBottom: 24 }}>Are you sure you want to delete this feature?</p>
              <div style={s.saveBtnRow}>
