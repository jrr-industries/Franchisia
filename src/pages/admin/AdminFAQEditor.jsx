...
export default function AdminFAQEditor() {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ question: "", answer: "", category: "", displayOrder: 0 });
    setShowModal(true);
  };
  ...
  const openEdit = (item) => {
    setEditing(item);
    setForm({
      question: item.question || "", answer: item.answer || "", category: item.category || "", displayOrder: item.displayOrder ?? 0,
    });
    setShowModal(true);
  };
              ...
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 600, margin: 0 }}>{item.question}</h3>
                    ...
                    <span style={{ fontSize: 11, color: "var(--text-muted)" }}>Order: {item.displayOrder}</span>
                  </div>
                  <p style={{ fontSize: 13, color: "var(--text-secondary)", marginTop: 4, marginBottom: 0 }}>{item.answer}</p>
                </div>
                <div style={s.actions}>
