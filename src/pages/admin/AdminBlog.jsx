...
const s = {
  ...
  imagePreview: { width: 80, height: 80, borderRadius: 8, objectFit: "cover", border: "1px solid var(--border)" },
...
export default function AdminBlog() {
  ...
  const [form, setForm] = useState({
    ...
    status: "draft", isFeatured: false, publishDate: "",
  });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ title: "", slug: "", featuredImage: "", author: "", category: "", tags: "", shortDescription: "", content: "", seoTitle: "", seoDescription: "", status: "draft", isFeatured: false, publishDate: "" });
    setShowModal(true);
  };
    ...
    <div style={s.page}>
      ...
      <div style={s.searchBar}>
        <div style={{ position: "relative", flex: 1 }}>
          ...
          <input style={{ ...s.searchInput, paddingLeft: 36 }} placeholder="Search by title or author..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
            ...
            <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} style={s.card}>
              <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0 }}>{item.author}{item.category ? ` · ${item.category}` : ""}{item.publishDate ? ` · ${new Date(item.publishDate).toLocaleDateString()}` : ""}</p>
              ...
              <div style={s.formRow}>
                ...
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input type="datetime-local" style={s.input} value={form.publishDate} onChange={(e) => setForm((prev) => ({ ...prev, publishDate: e.target.value }))} />
                  </div>
                </div>
              </div>
