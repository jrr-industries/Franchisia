...
export default function AdminPlans() {
  ...
  const [form, setForm] = useState({
    name: "", slug: "", price: "", interval: "month", description: "",
    features: "", isPopular: false, displayOrder: 0, status: "draft",
  });
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ name: "", slug: "", price: "", interval: "month", description: "", features: "", isPopular: false, displayOrder: items.length + 1, status: "draft" });
    setShowModal(true);
  };
  ...
  const openEdit = (item) => {
    ...
    setForm({
      ...
      features: item.features ? JSON.stringify(parseFeatures(item.features), null, 2) : "",
      ...
      <AnimatePresence mode="popLayout">
        {items.length === 0 ? (
          <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={s.emptyState}>
            <p style={{ fontSize: 16 }}>No plans found</p>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    ...
                    {item.isPopular && <span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, backgroundColor: "#DBEAFE", color: "#1E40AF" }}>Popular</span>}
                  </div>
                  <p style={{ fontSize: 13, color: "var(--text-muted)", margin: 0 }}>
                    ${parseFloat(item.price).toFixed(2)} / {item.interval} · Order: {item.displayOrder ?? 0}
                  </p>
                </div>
                <div style={s.actions}>
        ...
        {showModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} style={s.modalContent} onClick={(e) => e.stopPropagation()}>
              ...
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
                    ...
                    <input type="number" step="0.01" style={s.input} value={form.price} onChange={(e) => setForm((prev) => ({ ...prev, price: e.target.value }))} />
                  </div>
                </div>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
              ...
              <div style={s.fieldGroup}>
                <label style={s.label}>Features (JSON array or one per line)</label>
                <textarea style={{ ...s.textarea, minHeight: 120 }} value={form.features} onChange={(e) => setForm((prev) => ({ ...prev, features: e.target.value }))} placeholder='["Feature 1", "Feature 2"]' />
              </div>
              <div style={s.formRow}>
                <div style={s.halfField}>
                  <div style={s.fieldGroup}>
              ...
              <div style={s.checkboxRow}>
                ...
                <label htmlFor="isPopular" style={s.label}>Popular Plan</label>
              </div>
        ...
        {deleteId && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={s.modal} onClick={() => setDeleteId(null)}>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} style={{ ...s.modalContent, maxWidth: 400 }} onClick={(e) => e.stopPropagation()}>
              ...
              <p style={{ fontSize: 14, color: "var(--text-secondary)", marginBottom: 24 }}>Are you sure you want to delete this plan?</p>
              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
